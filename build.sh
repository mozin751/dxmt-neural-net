../meson-1.10.0/meson.py setup --cross-file build-win64-sys.txt --native-file build-osx.txt -Dwine_builtin_dll=true -Dnative_llvm_path=toolchains/llvm-darwin -Dwine_install_path=toolchains/wine -Denable_nvapi=true -Ddxmt_debug=true -Denable_nvngx=true build --buildtype release --prefix $(pwd)/dxmtBuild --strip --reconfigure
